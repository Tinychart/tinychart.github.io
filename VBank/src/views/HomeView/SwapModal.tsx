...
const Amount = ({ coin, amount = 0 }) => {
    return (
        <div className="flex items-center space-x-2">
            <AnimatedText className="font-mono" text={amount.toFixed(6)} />
            <Ticker>{coin}</Ticker>
        </div>
    );
};
...
const SwapModal: FC<SwapModalProps> = ({ modalOpen, closeModal, action, coin, price }) => {
        ...
        if (wallet) {
            ...
            await API.get(
                ...
                (data) => {
                    ...
                    cBalance = (asset?.amount || 0) / Math.pow(10, 6);
    ...
    const calculateOutput = () => {
        return inputNumber * (action === 'Mint' ? 0.99 : 1) * (action === 'Burn' ? 1 / price : price);
    };
    ...
    const doAction = async () => {
        ...
        try {
            ...
            await contract.apis.Claimant[`${action.toLowerCase()}${coin === 'vUSD' ? 'Stable' : 'Reserve'}`](
                reach.parseCurrency(inputNumber),
            );
        } catch (e) {
            message(e.toString());
        }
        const [aBalance, cBalance] = await updateBalance();
        setSwapOutput(action == 'Mint' ? cBalance - coinBalance : aBalance - algoBalance);
                    ...
                    <div className="bg-white bg-opacity-5 rounded-lg divide-y divide-grey-800">
                        <div className="p-4 w-full md:flex justify-between">
                            ...
                            <div className="flex justify-end">
                                <Amount coin={action === 'Burn' ? coin : 'ALGO'} amount={inputNumber} />
                            </div>
                        </div>
                        <div className="p-4 w-full md:flex justify-between">
                            <div className="font-bold flex items-center space-x-2">
