    ...
    const updateStates = async () => {
        if (wallet) {
            try {
                ...
                const reservesDivScCirc = reserves / stableCirculation;

                const priceStable = () => {
                    if (stableCirculation == 0) {
                        return targetPriceSC;
                    } else {
                        return reservesDivScCirc > targetPriceSC ? targetPriceSC : reservesDivScCirc;
                    }
                };

                const liabilities = stableCirculation * priceStable();

                const reserveRatio = reserves / liabilities;

                const equity = reserves - liabilities;

                const tgtReservePrice = equity / reserveCirculation;

                const priceReserve = () => {
                    if (reserveCirculation == 0 || equity == 0) {
                        return minPriceRC;
                    } else {
                        return tgtReservePrice > minPriceRC ? tgtReservePrice : minPriceRC;
                    }
                };

                setStablePrice(priceStable() / Math.pow(10, 6));
                setStableCirculating(stableCirculation / Math.pow(10, 6));
                setReservePrice(priceReserve() / Math.pow(10, 6));
                setReserveCirculating(reserveCirculation / Math.pow(10, 6));
                    ...
                    <Card rootClassName="w-64">
                        <div className="py-8 flex flex-col items-center space-y-8 justify-around text-xl">
                            <div className="items-center flex flex-col justify-center">
                                ...
                                <span className="font-mono">
                                    <AnimatedText text={formatPrice(reservePercent, 2)} />%
                                </span>
                            </div>
                            <div className="items-center flex flex-col justify-center">
